...
def save_processed_file(data: Dict[str, Any], target_path: Path) -> bool:
    ...
    if file_changed:
        ...
        yaml_dumper.default_style = '|'  # Use literal block scalar for multiline strings
...
def pull_file(repo_config: RepoConfig, file_config: FileConfig, translations_dir: Path) -> bool:
    """
    ...
    try:
        ...
        if not strings_only:
            print(f"  Warning: No translatable strings found in {file_config['source']}")
            return False
...
def commit_changes(updated_files: List[str]) -> None:
    ...
    try:
        ...
        commit_message = f"chore: update source translation files\n\nUpdated files:\n" + "\n".join(f"- {f}" for f in updated_files)
